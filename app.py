...
def layout(request, title, body, extra_script=""):
    ...
    async function pollAnkaState(){
      if(ankaBusy||document.hidden)return;
      ankaBusy=true;
      try{
        const r=await fetch("/api/state",{cache:"no-store",headers:{"X-Requested-With":"ANKA"}});
        if(!r.ok)return;
        const d=await r.json();
        if(ankaStateKey===null){ankaStateKey=d.key;return;}
        if(d.key!==ankaStateKey){
          ankaStateKey=d.key;
          if(["assignment","cancel_request","approval","help"].includes(d.event)){
            beep();
            const msg={assignment:"Yeni seans atandı.",cancel_request:"Yeni iptal talebi geldi.",approval:"Yeni onay bekleyen işlem var.",help:"Personel yardım istiyor."}[d.event];
            if("Notification"in window&&Notification.permission==="granted")new Notification("ANKA",{body:msg});
          }
          if(Date.now()-ankaReloadAt>1500){ankaReloadAt=Date.now();setTimeout(()=>location.reload(),500);}
        }
      }catch(e){}finally{ankaBusy=false;}
    }
    document.addEventListener("click",requestNoticePermission,{once:true});
    document.addEventListener("visibilitychange",()=>{if(!document.hidden)pollAnkaState();});
    window.addEventListener("focus",pollAnkaState);
    pollAnkaState();setInterval(pollAnkaState,3000);if("serviceWorker" in navigator){navigator.serviceWorker.register("/service-worker.js").catch(()=>{});}
    </script>
    """
    return f"""<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#0b1220"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="black-translucent"><meta name="apple-mobile-web-app-title" content="ANKA"><link rel="manifest" href="/manifest.webmanifest"><link rel="icon" href="/app-icon.svg"><title>{title} • ANKA</title><style>{CSS}</style></head><body>
