...
def layout(request, title, body, extra_script=""):
    ...
    if u["role"] == "staff":
        ...
        nav = f"""<nav>
          ...
          <a href="/reports"><b>▥</b><span>Raporlar</span></a>
...
def dashboard(request: Request):
    ...
    if u["role"] == "staff":
        ...
        for r in today_rows:
            ...
            rows += (
                ...
                f"<td>{ttime(r['started_at']) if r['status']!='assigned' else '—'}</td><td>{ttime(exit_time)}</td>"
    ...
    cards, total_rev, total_active, total_pending, total_assigned = [], 0, 0, 0, 0
    for branch in visible:
        ...
        cards.append(
            f"<article class='card'><h3>{branch['name']}</h3><p class='muted'>Personel bekliyor: {x['assigned'] or 0}</p><p class='muted'>Aktif: {x['active'] or 0}</p><p class='muted'>Onay bekleyen: {x['pending'] or 0}</p><div class='amount'>{money(x['rev'])}</div><a class='btn' href='/live?branch={branch['id']}'>Seans Durumu</a></article>"
        )
    con.close()
    ...
    <div class="stats"><div class="stat"><small>Toplam Ciro</small><strong>{money(total_rev)}</strong></div><div class="stat"><small>Personel Bekliyor</small><strong>{total_assigned}</strong></div><div class="stat"><small>Aktif Seans</small><strong>{total_active}</strong></div><div class="stat warn"><small>Onay Bekleyen</small><strong>{total_pending}</strong></div></div>
...
def session_new(
    ...
    cur = con.execute(
        """INSERT INTO sessions(session_no,branch_id,room_id,service_id,staff_id,amount_kurus,note,status,started_at,ends_at,assigned_at,business_date)
           VALUES(?,?,?,?,?,?,?,'assigned',?,?,?,?)""",
        (no, branch_id, room_id, service_id, staff_id, amount_k, note[:500], iso_utc(assigned), iso_utc(assigned), iso_utc(assigned), business_date()),
    )
    con.commit()
    sid = cur.lastrowid
    con.close()
    audit(request, "assign", "session", sid, no)
...
def cancel_session(request: Request, sid: int, csrf_token: str = Form(..., alias="csrf")):
    ...
    if not u or not csrf_ok(request, csrf_token):
        return RedirectResponse("/", 303)
    cancelled = iso_utc(utc_now())
    con = db()
    con.execute(
        """UPDATE sessions SET status='cancelled',cancelled_at=?,completed_at=?,
           cancel_reason='Personel iptal etti',commission_kurus=0
           WHERE id=? AND staff_id=? AND status IN('assigned','active')""",
        (cancelled, cancelled, sid, u["id"]),
    )
    con.commit()
    con.close()
    audit(request, "cancel", "session", sid, "Personel iptal etti")
    flash(request, "Seans iptal edildi; ciro ve prime eklenmedi.")
    return RedirectResponse("/", 303)
...
def live(request: Request, branch: Optional[int] = None):
    ...
    for room in rooms:
        s = con.execute(
            """SELECT s.*,u.full_name staff,sv.name service,sv.duration_minutes
               ...
               WHERE s.room_id=? AND s.status IN('assigned','active') ORDER BY s.id DESC LIMIT 1""",
            (room["id"],),
        ).fetchone()
        if s and s["status"] == "assigned":
            cards.append(
                f"""<article class="card room-card"><h3>{room['name']}</h3><span class="badge pending">Personel Bekleniyor</span>
                <p>{s['staff']} • {s['service']} • {s['duration_minutes']} dk</p><div class="room-free">Henüz başlamadı</div></article>"""
            )
        elif s:
    ...
    body = f"""<div class="head"><div><h1>Seans Durumu</h1><p>Canlı oda, personel kabulü ve sayaç ekranı</p></div><form method="get"><select name="branch" onchange="this.form.submit()">{options}</select></form></div><div class="room-grid">{''.join(cards)}</div>"""
    script = """
    <script>
    let notified={};
    function tickLive(){
...
def day_end_page(request: Request, branch_id: Optional[int] = None, day: Optional[str] = None):
    ...
    grouped = {}
    for s in sessions:
        grouped.setdefault(s["staff"], []).append(s)
    ...
    staff_cards = ""
    for staff_name, items in grouped.items():
        approved_total = sum(i["amount_kurus"] for i in items if i["status"] == "approved")
        commission_total = sum(i["commission_kurus"] for i in items if i["status"] == "approved")
        rows = "".join(
            f"<tr><td>{i['service']}</td><td>{i['room']}</td><td>{clock(i['started_at'])}</td><td>{clock(i['completed_at'] or i['cancelled_at'])}</td><td>{money(i['amount_kurus'])}</td><td><span class='badge {i['status']}'>{STATUS.get(i['status'],i['status'])}</span></td></tr>"
            for i in items
        )
        staff_cards += f"""<section class="card"><h3>{staff_name}</h3><div class="table"><table><tr><th>Seans</th><th>Oda</th><th>Giriş</th><th>Çıkış</th><th>Tutar</th><th>Durum</th></tr>{rows}</table></div>
        <p><strong>Onaylı ciro:</strong> {money(approved_total)} &nbsp; <strong>Prim:</strong> {money(commission_total)}</p></section>"""
    if not staff_cards:
        staff_cards = "<section class='card'><p>Bu gün için seans bulunmuyor.</p></section>"

    expense_rows = "".join(
        f"<tr><td>{e['description']}</td><td>{money(e['amount_kurus'])}</td><td>{e['creator']}</td></tr>"
        for e in expenses
    ) or "<tr><td colspan='3'>Masraf yok.</td></tr>"

    branch_options = "".join(
        f"<option value='{b['id']}' {'selected' if b['id']==branch_id else ''}>{b['name']}</option>"
        for b in branches
    )
    blockers = int(totals["active"] or 0) + int(totals["assigned"] or 0) + int(totals["pending"] or 0)
    closing_box = ""
    if closing and not closing["reopened_at"]:
        cash_diff = closing["counted_cash_kurus"] - closing["expected_cash_kurus"]
        card_diff = closing["counted_card_kurus"] - closing["expected_card_kurus"]
        iban_diff = closing["counted_iban_kurus"] - closing["expected_iban_kurus"]
        closing_box = f"""<section class="card"><h2>Gün Kapatıldı</h2><p>{closing['closed_at']}</p>
        <div class="stats"><div class="stat"><small>Nakit farkı</small><strong>{money(cash_diff)}</strong></div><div class="stat"><small>Kart farkı</small><strong>{money(card_diff)}</strong></div><div class="stat"><small>IBAN farkı</small><strong>{money(iban_diff)}</strong></div></div>
        ...
        <label>Sayılan Nakit<input name="cash" value="0" required></label><label>POS Toplamı<input name="card" value="0" required></label><label>IBAN/Banka Toplamı<input name="iban" value="0" required></label><label>Not<textarea name="note"></textarea></label>
        <button class="btn primary" {'disabled' if blockers else ''}>Günü Kapat ve Kilitle</button></form></section>"""

    body = f"""<div class="head"><div><h1>Gün Sonu</h1><p>Personel sütunları, masraflar ve kasa mutabakatı</p></div>
    ...
    <div class="stats"><div class="stat"><small>Onaylı Ciro</small><strong>{money(totals['revenue'])}</strong></div><div class="stat"><small>Nakit</small><strong>{money(totals['cash'])}</strong></div><div class="stat"><small>Kart</small><strong>{money(totals['card'])}</strong></div><div class="stat"><small>IBAN</small><strong>{money(totals['iban'])}</strong></div><div class="stat"><small>Toplam Prim</small><strong>{money(totals['commission'])}</strong></div><div class="stat warn"><small>Masraf</small><strong>{money(expense_total)}</strong></div></div>
    <div class="grid"><section><h2>Personel Hesapları</h2>{staff_cards}</section>
    <section><section class="card"><h2>Masraflar</h2><form method="post" action="/expenses/new" class="form"><input type="hidden" name="csrf" value="{csrf(request)}"><input type="hidden" name="branch_id" value="{branch_id}"><input type="hidden" name="day" value="{day}"><label>Açıklama<input name="description" required></label><label>Tutar<input name="amount" required></label><button class="btn primary">Masraf Ekle</button></form><div class="table"><table><tr><th>Açıklama</th><th>Tutar</th><th>Ekleyen</th></tr>{expense_rows}</table></div></section>{closing_box}</section></div>"""
...
def reports_page(request: Request, period: str = "week", anchor: Optional[str] = None, branch_id: Optional[int] = None):
    ...
    net = totals["revenue"] - expense_total - totals["commission"]
    ...
    <div class="stats"><div class="stat"><small>Giren Para</small><strong>{money(totals['revenue'])}</strong></div><div class="stat warn"><small>Çıkan Masraf</small><strong>{money(expense_total)}</strong></div><div class="stat"><small>Personel Primi</small><strong>{money(totals['commission'])}</strong></div><div class="stat"><small>Net</small><strong>{money(net)}</strong></div><div class="stat"><small>Onaylı Seans</small><strong>{totals['approved_count']}</strong></div><div class="stat"><small>İptal</small><strong>{totals['cancelled_count']}</strong></div><div class="stat"><small>En Çok Tercih Edilen Personel</small><strong>{top_staff}</strong></div><div class="stat"><small>En Çok Tercih Edilen Seans</small><strong>{top_service}</strong></div></div>
