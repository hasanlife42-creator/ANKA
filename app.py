...
def migrate(con):
    ...
    CREATE TABLE IF NOT EXISTS staff_deductions(
      ...
      deduction_type TEXT NOT NULL CHECK(deduction_type IN('meal','cleaning','custom')),
      description TEXT,
      amount_kurus INTEGER NOT NULL,
      created_by INTEGER NOT NULL REFERENCES users(id),
      created_at TEXT NOT NULL,
...
def approvals(request: Request):
    ...
    con.close()
    items = "".join(
        f"""<article class="approval"><div><span class="badge pending">Onay Bekliyor</span><h3>{r['staff']}</h3><p>{r['branch']} • {r['room']} • {r['service']}</p><p><strong>Ödeme:</strong> {PAYMENT.get(r['payment_method'],'—')}</p></div>
...
def day_end_page(request: Request, branch_id: Optional[int] = None, day: Optional[str] = None):
    ...
    for staff in staff_list:
        ...
        custom = sum(d["amount_kurus"] for d in staff_deds if d["deduction_type"] == "custom")
        total_ded = meal + cleaning + custom
        net_commission = gross_commission - total_ded
        ...
        custom_list = "".join(
            f"<li>{d['description'] or 'Diğer masraf'}: {money(d['amount_kurus'])}</li>"
            for d in staff_deds if d["deduction_type"] == "custom"
        )
        ...
        <p><strong>Yemek:</strong> -{money(meal)}</p>
        <p><strong>Temizlik:</strong> -{money(cleaning)}</p>
        <p><strong>Diğer Kesinti:</strong> -{money(custom)}</p>
        {f"<ul>{custom_list}</ul>" if custom_list else ""}
        <p style="font-size:20px"><strong>Net Prim:</strong> {money(net_commission)}</p>
        <form method="post" action="/staff-deductions/new" class="form">
          <input type="hidden" name="csrf" value="{csrf(request)}">
          <input type="hidden" name="branch_id" value="{branch_id}">
          <input type="hidden" name="staff_id" value="{staff['id']}">
          <input type="hidden" name="day" value="{day}">
          <label>Kesinti Türü<select name="deduction_type"><option value="meal">Yemek Parası</option><option value="cleaning">Temizlik Parası</option><option value="custom">Diğer Masraf</option></select></label>
          <label>Açıklama<input name="description" placeholder="Diğer masraf için açıklama"></label>
          <label>Tutar<input name="amount" required></label>
          <button class="btn">Personelden Düş</button>
        </form></section>"""
    ...
    body = f"""<div class="head"><div><h1>Gün Sonu</h1><p>Defter/Excel düzeninde personel bazlı hesap</p></div>
    ...
    <div class="stats"><div class="stat"><small>Onaylı Ciro</small><strong>{money(totals['revenue'])}</strong></div><div class="stat"><small>Nakit</small><strong>{money(totals['cash'])}</strong></div><div class="stat"><small>Kart</small><strong>{money(totals['card'])}</strong></div><div class="stat"><small>IBAN</small><strong>{money(totals['iban'])}</strong></div><div class="stat"><small>Toplam Prim</small><strong>{money(totals['commission'])}</strong></div><div class="stat warn"><small>İşletme Masrafı</small><strong>{money(expense_total)}</strong></div></div>
...
def reports_page(request: Request, period: str = "week", anchor: Optional[str] = None, branch_id: Optional[int] = None):
    ...
    deduction_total = con.execute(
        f"SELECT COALESCE(SUM(amount_kurus),0) total FROM staff_deductions WHERE deduction_date BETWEEN ? AND ? AND active=1 {exp_clause}",
        exp_args,
    ).fetchone()["total"]
    ...
    <div class="stats"><div class="stat"><small>Giren Para</small><strong>{money(totals['revenue'])}</strong></div><div class="stat warn"><small>İşletme Masrafı</small><strong>{money(expense_total)}</strong></div><div class="stat"><small>Personel Primi</small><strong>{money(totals['commission'])}</strong></div><div class="stat"><small>Personel Kesintileri</small><strong>{money(deduction_total)}</strong></div><div class="stat"><small>Net</small><strong>{money(net)}</strong></div><div class="stat"><small>Onaylı Seans</small><strong>{totals['approved_count']}</strong></div><div class="stat"><small>En Çok Tercih Edilen Personel</small><strong>{top_staff}</strong></div><div class="stat"><small>En Çok Tercih Edilen Seans</small><strong>{top_service}</strong></div></div>
