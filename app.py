...
def audit(request, action, entity, entity_id="", details=""):
    ...
    for attempt in range(4):
        ...
        try:
            con = db()
            con.execute("INSERT INTO audit_logs(user_id,action,entity_type,entity_id,details,ip_address,created_at) VALUES(?,?,?,?,?,?,?)", (u["id"] if u else None, action, entity, str(entity_id), str(details), ip, local_now_text()))
            con.commit(); return True
        except sqlite3.OperationalError as exc:
            if "locked" not in str(exc).lower() or attempt == 3: return False
...
def layout(request, title, body, extra_script=""):
    ...
    async function requestNoticePermission(){
      if(!("Notification" in window)){alert("Bu tarayıcı bildirimleri desteklemiyor. iPhone'da ANKA'yı Ana Ekrana Ekle ile açmayı deneyin.");return false;}
      try{const p=await Notification.requestPermission();if(p==="granted"){beep();alert("Bildirim ve ses izni aktif.");return true;}if(p==="denied"){alert("Bildirim izni reddedilmiş. Tarayıcı/site ayarlarından ANKA için izin verin.");}return false;}catch(e){alert("Bildirim izni açılamadı. iPhone'da ANKA'yı Ana Ekrana ekleyip tekrar deneyin.");return false;}
    }
    function beep(){try{const C=window.AudioContext||window.webkitAudioContext;if(!C)return;const ctx=new C(),o=ctx.createOscillator(),g=ctx.createGain();o.connect(g);g.connect(ctx.destination);o.frequency.value=880;g.gain.value=.18;o.start();setTimeout(()=>{o.stop();ctx.close()},700)}catch(e){}}
    function lockSubmit(form){const b=form.querySelector('button[type=submit],button:not([type])');if(b){b.disabled=true;b.textContent='İşleniyor…';}return true;}
    let ankaStateKey=null,ankaBusy=false,ankaReloadAt=0;
    async function pollAnkaState(){if(ankaBusy||document.hidden)return;ankaBusy=true;try{const r=await fetch('/api/state',{cache:'no-store'});if(!r.ok)return;const d=await r.json();if(ankaStateKey===null){ankaStateKey=d.key;return}if(d.key!==ankaStateKey){ankaStateKey=d.key;beep();const m={assignment:'Yeni seans atandı.',cancel_request:'Yeni iptal talebi geldi.',approval:'Yeni onay bekleyen işlem var.',help:'Personel yardım istiyor.',update:'ANKA’da yeni bir işlem var.'}[d.event]||'ANKA’da yeni bir işlem var.';if('Notification'in window&&Notification.permission==='granted')new Notification('ANKA',{body:m});if(Date.now()-ankaReloadAt>1500){ankaReloadAt=Date.now();setTimeout(()=>location.reload(),500)}}}catch(e){}finally{ankaBusy=false}}
    document.addEventListener('visibilitychange',()=>{if(!document.hidden)pollAnkaState()});window.addEventListener('focus',pollAnkaState);pollAnkaState();setInterval(pollAnkaState,3000);if('serviceWorker'in navigator)navigator.serviceWorker.register('/service-worker.js').catch(()=>{});
    </script>"""
...
async def validation_error_handler(request: Request, exc: RequestValidationError):
    u=current_user(request)
    msg="İşlem tamamlanamadı; gönderilen bilgiler eksik veya geçersiz."
    fields=[]
    for err in exc.errors():
        loc=[str(x) for x in err.get("loc",()) if x not in ("body","query","path")]
        if loc: fields.append("/".join(loc))
    if fields: msg += " Kontrol edin: " + ", ".join(fields[:4])
    if u:
        flash(request,msg,"error")
        ref=request.headers.get("referer",""); base=str(request.base_url).rstrip("/")
        return RedirectResponse(ref if ref.startswith(base) else "/",303)
    return HTMLResponse("<h2>ANKA</h2><p>İşlem tamamlanamadı.</p><a href='/login'>Giriş</a>",status_code=400)
        ...
        for room in rooms:
            ...
            if s and s["status"] == "assigned":
                ...
                cards.append(
                    ...
                    <div class="inline">
                      <form method="post" action="/session/{s['id']}/extend" class="inline" style="gap:6px">
                        <input type="hidden" name="csrf" value="{csrf(request)}">
                        <input type="number" name="minutes" value="15" min="1" max="180"
                               inputmode="numeric" style="width:84px" aria-label="Eklenecek dakika">
                        <button class="btn">Süre Ekle</button>
                      </form>
...
def extend(request: Request, sid: int, minutes: int = Form(...), csrf_token: str = Form(..., alias="csrf")):
    ...
    con = db()
    s = con.execute(
        "SELECT * FROM sessions WHERE id=? AND status IN('active','pending')",
        (sid,),
    ).fetchone()

    if not s:
        con.close()
        flash(request, "Süre eklenebilecek seans bulunamadı.", "error")
        return RedirectResponse("/live", 303)
    ...
    if day_is_closed(s["branch_id"], s["business_date"]):
        con.close()
        flash(request, "Kapalı güne süre eklenemez.", "error")
        return RedirectResponse("/day-end", 303)

    base = datetime.fromisoformat(s["ends_at"].replace("Z", "+00:00")) if s["ends_at"] else utc_now()
    if base < utc_now():
        base = utc_now()
    ...
    con.execute(
        """UPDATE sessions
           SET ends_at=?, status='active', completed_at=NULL,
               updated_at=?, updated_by=?
           WHERE id=?""",
        (iso_utc(new_end), local_now_text(), u["id"], sid),
    )
    con.commit()
    con.close()

    audit(request, "extend", "session", sid, f"+{minutes} dk")
    flash(request, f"Seansa {minutes} dakika eklendi.")
    return RedirectResponse("/live", 303)
...
def cancel_decision(request: Request, sid: int, decision: str = Form(...), pay_commission: int = Form(0), csrf_token: str = Form(..., alias="csrf")):
    u=require(request,{"admin","manager"})
    if not u or not csrf_ok(request,csrf_token): flash(request,"Oturum doğrulanamadı.","error"); return RedirectResponse("/approvals",303)
    if decision not in {"approve","reject"}: flash(request,"Geçersiz karar.","error"); return RedirectResponse("/approvals",303)
    con=db(); action=None; detail=""
    try:
        row=con.execute("SELECT s.*,u.commission_rate FROM sessions s JOIN users u ON u.id=s.staff_id WHERE s.id=? AND s.status='cancel_requested'",(sid,)).fetchone()
        if not row: flash(request,"Bu talep daha önce işlenmiş veya artık beklemede değil.","error"); return RedirectResponse("/approvals",303)
        if u["role"]=="manager" and row["branch_id"]!=u["branch_id"]: flash(request,"Bu şube için yetkiniz yok.","error"); return RedirectResponse("/approvals",303)
        now=iso_utc(utc_now())
        if decision=="approve":
            pc=1 if int(pay_commission)==1 else 0; commission=0
            if pc: commission=int((Decimal(row["amount_kurus"])*Decimal(row["commission_rate"])/100).quantize(Decimal("1"),rounding=ROUND_HALF_UP))
            cur=con.execute("UPDATE sessions SET status='cancelled',cancelled_at=?,completed_at=?,commission_kurus=?,cancel_pay_commission=?,cancel_decision_at=?,cancel_decision_by=?,help_requested=0,updated_at=?,updated_by=? WHERE id=? AND status='cancel_requested'",(now,now,commission,pc,now,u["id"],local_now_text(),u["id"],sid)); msg="İptal onaylandı."; action="cancel_approved"; detail=f"prime_yaz={pc}"
        else:
            fallback="active" if row["accepted_at"] else "assigned"
            cur=con.execute("UPDATE sessions SET status=?,cancel_decision_at=?,cancel_decision_by=?,cancel_pay_commission=NULL,updated_at=?,updated_by=? WHERE id=? AND status='cancel_requested'",(fallback,now,u["id"],local_now_text(),u["id"],sid)); msg="İptal talebi reddedildi; seans önceki durumuna döndü."; action="cancel_rejected"; detail=fallback
        if cur.rowcount!=1: con.rollback(); flash(request,"İşlem başka bir cihaz tarafından zaten yapılmış.","error"); return RedirectResponse("/approvals",303)
        con.commit()
    except sqlite3.OperationalError as exc:
        con.rollback(); flash(request,"Veritabanı meşgul. Birkaç saniye sonra tekrar deneyin." if "locked" in str(exc).lower() else "İşlem sırasında veritabanı hatası oluştu.","error"); return RedirectResponse("/approvals",303)
    except Exception:
        con.rollback(); flash(request,"İptal kararı işlenemedi; kayıt değiştirilmedi.","error"); return RedirectResponse("/approvals",303)
    finally: con.close()
    audit(request,action,"session",sid,detail); flash(request,msg); return RedirectResponse("/approvals",303)
...
def approvals(request: Request):
    ...
    for r in rows:
        if r["status"] == "cancel_requested":
            ...
            <div class="form">
              <form method="post" action="/session/{r['id']}/cancel-decision" class="form" onsubmit="return lockSubmit(this)">
                <input type="hidden" name="csrf" value="{csrf(request)}"><input type="hidden" name="decision" value="approve">
                ...
                <button type="submit" class="btn red">İptali Onayla</button>
              </form>
              <form method="post" action="/session/{r['id']}/cancel-decision" onsubmit="return lockSubmit(this)">
                <input type="hidden" name="csrf" value="{csrf(request)}"><input type="hidden" name="decision" value="reject"><input type="hidden" name="pay_commission" value="0">
                <button type="submit" class="btn full">İptali Reddet</button>
              </form>
            </div></article>"""
        else:
...
def approve(request: Request, sid: int, csrf_token: str = Form(..., alias="csrf")):
    u=require(request,{"admin","manager"})
    if not u or not csrf_ok(request,csrf_token): return RedirectResponse("/approvals",303)
    con=db(); payment=""
    try:
        row=con.execute("SELECT s.*,u.commission_rate FROM sessions s JOIN users u ON u.id=s.staff_id WHERE s.id=? AND s.status='pending'",(sid,)).fetchone()
        if not row: flash(request,"Bu işlem artık onay beklemiyor.","error"); return RedirectResponse("/approvals",303)
        if u["role"]=="manager" and row["branch_id"]!=u["branch_id"]: return RedirectResponse("/approvals",303)
        commission=int((Decimal(row["amount_kurus"])*Decimal(row["commission_rate"])/100).quantize(Decimal("1"),rounding=ROUND_HALF_UP))
        cur=con.execute("UPDATE sessions SET status='approved',commission_kurus=?,approved_at=?,approved_by=?,updated_at=?,updated_by=? WHERE id=? AND status='pending'",(commission,iso_utc(utc_now()),u["id"],local_now_text(),u["id"],sid))
        if cur.rowcount!=1: con.rollback(); flash(request,"İşlem başka bir cihaz tarafından işlendi.","error"); return RedirectResponse("/approvals",303)
        payment=row["payment_method"] or ""; con.commit()
    finally: con.close()
    audit(request,"approve","session",sid,payment); flash(request,"İşlem onaylandı."); return RedirectResponse("/approvals",303)
...
def edit_room(request: Request, rid: int, name: str = Form(...), csrf_token: str = Form(..., alias="csrf")):
    ...
    if room and (u["role"] == "admin" or room["branch_id"] == u["branch_id"]):
        ...
        con.commit(); con.close(); audit(request, "edit", "room", rid, name); return RedirectResponse("/settings",303)
    con.close()
    return RedirectResponse("/settings", 303)
...
def toggle_room(request: Request, rid: int, csrf_token: str = Form(..., alias="csrf")):
    ...
    if room and (u["role"] == "admin" or room["branch_id"] == u["branch_id"]):
        ...
        con.commit(); con.close(); audit(request, "toggle", "room", rid, str(new_state)); return RedirectResponse("/settings",303)
    con.close()
    return RedirectResponse("/settings", 303)
...
def toggle_service(request: Request, sid: int, csrf_token: str = Form(..., alias="csrf")):
    ...
    if row:
        ...
        con.commit(); con.close(); audit(request, "toggle", "service", sid, str(state)); return RedirectResponse("/settings",303)
    con.close()
    return RedirectResponse("/settings", 303)
...
def day_end_close(request:Request,branch_id:int=Form(...),day:str=Form(...),csrf_token:str=Form(...,alias='csrf')):
...
def reports_page(request: Request, period: str = "week", anchor: Optional[str] = None, branch_id: Optional[str] = None):
    u = require(request, {"admin"})
    if not u:
        return RedirectResponse("/login", 303)
...
def user_toggle(request: Request, uid: int, csrf_token: str = Form(..., alias="csrf")):
    ...
    if row:
        ...
        con.commit(); con.close(); audit(request, "toggle", "user", uid, str(state)); return RedirectResponse("/users",303)
    con.close()
    return RedirectResponse("/users", 303)
...
def reports_export_csv(request: Request, period: str = "month", anchor: Optional[str] = None, branch_id: Optional[str] = None):
    if not require(request, {"admin"}):
        return RedirectResponse("/login", 303)
    anchor = anchor or business_date()
    start, end = period_range(period, anchor)
    ...
    if u["role"] == "staff":
        ...
        rows = con.execute(
            f"""SELECT id,status,branch_id,room_id,staff_id,assigned_at,accepted_at,
                       completed_at,cancelled_at,cancel_requested_at,cancel_reason,approved_at,
